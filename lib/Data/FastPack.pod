...
Its small footprint and easy encoding/decoding also make is a great format for
IPC.

It optionally (but highly recommended ) support name aliasing for messages.
...
There is only one reserved channel id, 0 , which is a meta data channel and as
a structure data transport at the application level. This channel payload is
JSON or MessagePack encoded.  If no payload is present, this is a namespace
reset, which will remove named id mappings so new ones can be generated from
future data
...
If a named message with no payload encoded, this serves as a deregistration of
the id and adds it to a pool of reusable ids.

A message with id 0 and no payload, deregisters all (clears) the name space.
