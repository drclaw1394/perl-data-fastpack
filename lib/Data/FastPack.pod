...
Static IDs are simple for small closes systems. However for larger system,
using dynamic is allows the scoping.  The mapping between names and IDS it
automatically applied whtn using a name space. Encoding a named id for the
first time in insert a entry in the local ecoding side table. It will also
inject a message with  the dynamic id and payload of the name into the stream.
This will be the first time this id is seen, and is a marker to update the
decoding end of the system.

If a named message with no payload is encoded, this serves as a deregistration
of the id and adds it to a pool of reusable ids.

For a bidirectional link, 4 name spaces are acutally use. The encoding end of
...
sync with the master from th encoding end. 
...
C<$buf>.  C<$buf> is aliased interally, and not copied. All need encoded
...
spliced out of the input array to allow the same array to be appedend to
externally
...
provided  (even if undefinded) to use this argument.
...
maniupulate the namespace table to ensure it is copy of the  encoding end of
the stream
...
Returns a name space structure for named ids. Seperate name spaces structure
